...
import { motion, useAnimation, useMotionValue, useSpring, useScroll, useTransform, useReducedMotion } from 'framer-motion'
import Spline from '@splinetool/react-spline'

// Choose between concierge-friendly 3D variants via env:
// VITE_SPLINE_VARIANT = 'abstract' | 'island' | 'vip-card'
// You can also override with VITE_SPLINE_SCENE for a custom scene URL
const VARIANT = import.meta.env.VITE_SPLINE_VARIANT || 'abstract'
const SCENES = {
  // Abstract floating glass/pearl shape (default)
  abstract: 'https://prod.spline.design/myxXfbNiwnbTpGFp/scene.splinecode',
  // Minimal stylized island placeholder (falls back to abstract if not provided)
  island: 'https://prod.spline.design/myxXfbNiwnbTpGFp/scene.splinecode',
  // VIP access card in glass (falls back to abstract if not provided)
  'vip-card': 'https://prod.spline.design/myxXfbNiwnbTpGFp/scene.splinecode'
}
const SPLINE_SCENE = import.meta.env.VITE_SPLINE_SCENE || SCENES[VARIANT] || SCENES.abstract

const gradients = (
  <div className="absolute inset-0">
    <div className="pointer-events-none absolute -top-24 left-1/2 -translate-x-1/2 h-72 w-[36rem] rounded-full bg-[radial-gradient(circle_at_center,rgba(34,211,238,0.45),transparent_60%)] blur-3xl" />
    ...
    <div className="pointer-events-none absolute top-1/3 left-0 h-64 w-64 -translate-x-24 rounded-full bg-[radial-gradient(circle_at_center,rgba(20,184,166,0.32),transparent_60%)] blur-3xl" />
  </div>
)
...
export default function Hero() {
  ...
  // Parallax on scroll for content
  const { scrollY } = useScroll()
  const contentY = useTransform(scrollY, [0, 400], [0, prefersReducedMotion ? 0 : -24])
  const glowOpacity = useTransform(scrollY, [0, 300], [1, 0.6])
  ...
  return (
    <section className="relative min-h-[100dvh] overflow-hidden bg-black text-white">
      ...
      {/* Subtle loading veil for polished entry */}
      {!loaded && (
        <div className="pointer-events-none absolute inset-0 grid place-items-center">
          <div className="h-10 w-10 animate-pulse rounded-full bg-cyan-300/10 ring-1 ring-white/10" />
        </div>
      )}
      ...
      >
        <motion.div style={{ y: contentY }} className="w-full">
          <motion.h1
            ...
            className="text-center text-[28px] font-semibold tracking-tight sm:text-4xl"
          >
          ...
          <Footer glowOpacity={glowOpacity} />
        </motion.div>
      </div>
...
function Footer({ glowOpacity }) {
  return (
    <div className="mt-12 flex w-full max-w-md items-center justify-between text-xs text-cyan-200/60 mx-auto">
...
function Social({ icon, glowOpacity }) {
  return (
    <button className="group relative grid h-9 w-9 place-items-center rounded-full bg-white/5 backdrop-blur-md ring-1 ring-white/10 transition-transform active:scale-95">
      <motion.div className="absolute inset-0 rounded-full bg-cyan-400/0 blur-md" style={{ opacity: glowOpacity }} />
...
function WaitlistForm() {
  ...
  // Magnetic button effect (subtle, disabled during success)
